...
- [ ] T026 [P] [US1] `tests/unit/test_tasks_loader.py`: valid parse, unknown style dropped, malformed top-level fails fast (AC1.1, AC1.4, AC1.5).
- [ ] T027 [P] [US1] `tests/unit/test_ffmpeg.py`: WAV output is mono/16 kHz; ffmpeg failure raises typed error (AC1.2).
- [ ] T028 [P] [US1] `tests/unit/test_downloader.py`: successful fetch; unreachable URL records structured error, no raise (AC1.3).
- [ ] T029 [US1] `tests/integration/test_ingestion.py`: task → local video → WAV present in `CaptionState`; one bad task does not abort the batch (AC1.3).
